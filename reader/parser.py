...
def parse_rede(element) -> Rede:
    ...
    name = parse_name(element.find(".//redner/name"))
    fraktion = element.find(".//redner/name/fraktion")
    redner = Redner(redner_id, name, fraktion.text)
...
def parse_sitzung(xml_string: str) -> Sitzung:
    ...
    sitzung = Sitzung(
        wahlperiode=wahlperiode, sitzungsnummer=sitzungsnummer, datum=datum
    )
